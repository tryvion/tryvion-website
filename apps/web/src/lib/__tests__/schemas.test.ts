...
describe('contactSchema', () => {
  ...
  it('accepts when phone is omitted', () => {
    const withoutPhone: Omit<typeof valid, 'email'> & { email?: string } = {
      ...valid,
    };

    delete withoutPhone.email;
  ...
  it('rejects privacy: false (user has not accepted)', () => {
    ...
    if (!r.success) {
      expect(r.error.issues[0].message).toMatch(/privacy policy/i);
    }
  });

  it('rejects a missing required field', () => {
    const withoutEmail: Omit<typeof valid, 'email'> & {
      email?: string;
    } = {
      ...valid,
    };
...
describe('getStartedSchema', () => {
  ...
  it('rejects an empty services array', () => {
    const r = getStartedSchema.safeParse({
      ...valid,
      services: [],
    });
    ...
    if (!r.success) {
      expect(r.error.issues[0].path).toContain('services');
    }
  });
