...
export const metadata: Metadata = {
  title: 'Whitepapers',
  description: 'Research-backed whitepapers on enterprise AI readiness, SAP transformation, cloud strategy, and the future of digital operations.',
  alternates: { canonical: 'https://tryvion.com/insights/whitepapers' },
}
    ...
    <main style={{ background: '#050A18', minHeight: '100vh', color: '#fff' }}>
      <section style={{ padding: 'clamp(8rem, 14vw, 11rem) clamp(1.5rem, 5vw, 3.5rem) clamp(6rem, 8vw, 9rem)', maxWidth: '82rem', margin: '0 auto', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '1.75rem' }}>
          ...
          <span style={{ color: '#1458F2', fontSize: '0.8125rem', fontWeight: 600 }}>Whitepapers</span>
        </div>
        ...
        <p style={{ fontSize: 'clamp(1.125rem, 1.5vw, 1.3125rem)', color: 'rgba(255,255,255,0.55)', maxWidth: '44rem', lineHeight: 1.7, position: 'relative' }}>
          TRYVION research draws on data from hundreds of enterprise transformation programmes to give practitioners frameworks and evidence, not opinions.
        </p>
      </section>
      ...
      <section style={{ padding: '0 clamp(1.5rem, 5vw, 3.5rem) clamp(6rem, 8vw, 9rem)', maxWidth: '82rem', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {WHITEPAPERS.map((wp) => (
          <Link key={wp.slug} href={`/insights/whitepapers/${wp.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
            ...
            >
              <div>
                <div style={{ display: 'flex', gap: '0.625rem', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.75rem', fontWeight: 600, color: wp.accent, background: `${wp.accent}18`, padding: '0.25rem 0.75rem', borderRadius: '999px' }}>{wp.tag}</span>
                ...
                <h2 style={{ fontSize: '1.1875rem', fontWeight: 700, color: '#fff', lineHeight: 1.4, marginBottom: '0.75rem' }}>{wp.title}</h2>
                <p style={{ fontSize: '0.9375rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.75 }}>{wp.desc}</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.75rem', flexShrink: 0 }}>
                <div style={{ color: wp.accent, fontSize: '0.8125rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                  Download
